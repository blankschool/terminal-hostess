...
# Uso: ./scripts/dev/setup_macos.sh
...
if [[ "$OSTYPE" != "darwin"* ]]; then
    echo -e "${RED}Este script é para macOS. Para Linux/Ubuntu, use o docs/DEPLOY.md${NC}"
    exit 1
fi
...
if [ ! -f "config/cookies/cookies.txt" ]; then
    echo "Criando arquivo cookies.txt vazio em config/cookies..."
    touch config/cookies/cookies.txt
...
echo "  - Cookies: $([ -s config/cookies/cookies.txt ] && echo 'Configurado' || echo 'Vazio (OK para testes básicos)')"
echo ""
...
echo -e "${YELLOW}  ./scripts/dev/run_api.sh${NC}"
echo ""
echo "Ou manualmente:"
